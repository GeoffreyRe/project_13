...
class TranslationFile(models.Model):
    ...
    def analyze_content(self):
        """
        This method will analyse content of the file related
        with the instance of TranslationFile
        """
        ...
        with open(self.original_file.path, "r", encoding="utf-8") as f:
            ...
            for i, content_line in enumerate(content):
                ...
                if content[i] == "":
                    ...
                    last_block = i + 1
            # now we will check if there is the header and if there is
            # min 2 blocks (header + a traduction block)
            ...
            if len(blocks) < 2:
                raise FileParsingError(
                    "Aucun block de traduction n'a été trouvé dans le fichier {}".format(self.name)
                    )
            ...
            errors = TranslationBlock.check_errors_content(
                header_content, 1, file_name=self.name, is_header=True
                )
            if errors[0] is True:
            ...
            TranslationBlock.objects.create_block_from_data(
                header_content, file=self, is_header=True
                )
            ...
            for index, block_infos in enumerate(blocks[1:]):
                ...
                if block:
                    errors = TranslationBlock.check_errors_content(
                        block, block_infos['position'], file_name=self.name
                        )
                    if errors[0] is True:
                        raise FileParsingError(errors[1])
                    # if there is no errors, we will create the block
                    TranslationBlock.objects.create_block_from_data(
                        errors[1], block_position=block_infos['position'], file=self
                        )
...
def delete_file_associated_with_TranslationFile(sender, instance, **kwargs):
    ...
    """
    if (instance.original_file is not None):
        if os.path.exists(os.path.abspath(instance.original_file.path)):
            ...
            if os.path.isfile(absolute_path):
                ...
                try:
                    ...
                    logger.info("le fichier de l'instance {}"
                                " a bien été supprimée".format(instance))
                except OSError:
                    logger.error(
                        "Un problème est survenu lors"
                        " de la suppression du fichier de {}".format(instance))
            else:
                logger.error("Le chemin spécifié pour l'instance {}"
