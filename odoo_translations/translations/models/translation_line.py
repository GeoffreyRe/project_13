    ...
    def analyze_infos(self, data, block_position):
        ...
        if data['instance']:
            ...
            if len(instance_type) != 1:
                error_s = 'fichier {}, bloc ligne {} : {} --- problème avec le type d\'instance'
                raise FileParsingError(
                    error_s.format((file_name, block_position, data.get('line', ''))))
            ...
            try:
                ...
                error_string = 'Fichier: {}, bloc ligne {} : "{}" '.format(
                    file_name,
                    block_position,
                    data.get('line', '')
                    )
            ...
            instance_name_parts = instance_name.split('.') if instance_type_name != "code" \
                else instance_name.split(':')
            ...
            if instance_type_name == 'ir.model.fields':
                if not instance_name_parts[1].startswith('field_'):
                    error_string = 'Fichier: {}, bloc ligne {} : "{}" '.format(
                        file_name, block_position, data.get('line', '')
                        )
                    error_string += '\n"{}" ne commençant pas par "field_"'.format(
                        instance_name_parts[1]
                        )
                    raise FileParsingError(error_string)
                ...
                Instance.objects.filter(name=instance_name_parts[0],
                                        instance_type=module_instance_type,
                                        project=self.block.file.project)

                while found is False and len(model_field_parts) >= 1:
                ...
                if not instance_name_parts[1].startswith('model_'):
                    ...
                    error_string += '\n"{}" ne commençant pas par "model_"'.format(
                        instance_name_parts[1]
                        )
                    raise FileParsingError(error_string)

                self.find_or_create_instance(
                    instance_name_parts[1][6:], model_instance_type, instance_line_type
                    )

            elif instance_type_name == "module":
                self.find_or_create_instance(
                    instance_name_parts[0],
                    module_instance_type,
                    instance_line_type
                    )
