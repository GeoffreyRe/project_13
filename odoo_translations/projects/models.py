...
class Project(models.Model):
    ...
    # creator is a many-to-one field thats links
    # to the user who creates the project
    ...
    def modify_roles_of_user_on_project(self, values, user):
        for user_dict in values:
            if user_dict['id'] == "new":
                self.send_invitation_to_project(
                    user_dict['email'],
                    user_dict['role'],
                    inviting_user=user
                )
    ...
    def delete_files_of_project(self, files_ids, config_files=False):
        for file_id in files_ids:
            if config_files is True:
                try:
                    ConfigFileModel = apps.get_model('translations', 'ConfigFile')
                    file_to_delete = ConfigFileModel.objects.get(id=int(file_id))
                except ObjectDoesNotExist:
                    pass
            else:
                TranslationFileModel = apps.get_model('translations', 'TranslationFile')
    ...
    def analyze_translation_files(self):
        ...
        if len(self.translation_files.all()) == 0:
            ...
            raise FileParsingError(
                "Il n'y a pas de fichier de traduction à analyser pour ce projet"
                )
    ...
    def translations_instances(self, instance_id=False, with_children=True, type=False):
        """
        Retrieve all translations from instance
        associated with this project
        if instance_id is given,
        only find translations for this particular model
        if with_fields is True, then retrieve also
        translations of fields wich are 'children' of models
        ...
        if instance_id is not False:
            ...
            if with_children:
                # if with_children is True, then we add instances
                # that are children of model (its fields)
                children_instances = Instance.objects.filter(parent=instance)
                query = query | Q(instance__in=children_instances)

            blocks_lang = {}
            for lang in ['fr', 'ndlr']:
                blocks_of_lang = self.translation_files.filter(
                    translated_language=lang
                    ).values_list('translation_blocks', flat=True)
                translation_lines = TranslationLine.objects.filter(
                    query & Q(block__in=blocks_of_lang)
                    )
        ...
        if type is not False:
            ...
            if with_children:
                children_instances = Instance.objects.filter(
                    parent__in=[instance['instance'] for instance in instances])
                query = query | Q(instance__in=children_instances)
    ...
    def export_translations(self, lang):
        ...
        module_instances = apps.get_model(
            'translations.Instance'
            ).objects.filter(project=self, instance_type=module_type)
        ...
        module_instance_names = apps.get_model(
            'translations.Instance'
            ).objects.filter(
                id__in=module_instance_ids
                ).values_list('name', flat=True)
        modules_in_file = "\n".join(
            ["#       * " + module.strip() for module in module_instance_names]
            )
        ...
        name_list = ['ir.model', 'ir.ui.view', 'ir.ui.menu', 'ir.actions.act_window']
        for file in self.translation_files.filter(translated_language=lang):
            for block in file.translation_blocks.filter(is_header=False):
                module_line = block.translation_lines.filter(
                    instance__instance_type__name='module'
                    )
                module_name = module_line[0].instance.name.strip() if \
                    (module_line and module_line[0].instance) else ""
                for line in block.translation_lines.all():
                    if line.instance.instance_type.name == "module":
                        ...
                        content += "#: code:{}:{}".format(
                            line.instance.parent.name, line.instance.name
                            )
                    elif line.instance.instance_type.name in name_list:
                        content += "#: model:{},{}:{}.{}".format(line.instance.instance_type.name,
                                                                 line.line_type.name, module_name,
                                                                 line.instance.name)
                    elif line.instance.instance_type.name == 'ir.model.fields':
                        content += "#: model:{},{}:{}.field_{}_{}".format(
                                                                line.instance.instance_type.name,
                                                                line.line_type.name,
                                                                module_name,
                                                                line.instance.parent.name,
                                                                line.instance.name)
...
class Invitation(models.Model):
    ...
    def __str__(self):
        username = self.inviting_user.username
...
class UserProject(models.Model):
    ...
    def __str__(self):
        role_display_name = self.user_role.get_name_display()
